...
const AudioTimeline: React.FC = observer(() => {
  ...
  const [waveformInitialized, setWaveformInitialized] = useState(false);
  ...
  // Initialize waveform manager with container and audio URL
  useEffect(() => {
    if (
    ...
    // Load the audio URL only once the container is set
    const initWaveform = () => {
      ...
      if (audioUrl) {
        ...
        waveformManager.generateWaveform(audioUrl);
      }
    };
  ...
  }, [waveformManager, audioEditorManager, waveformInitialized]);

  // Update waveform when audio URL changes
  useEffect(() => {
    ...
    if (audioUrl) {
      console.log("Audio URL changed, regenerating waveform");
      waveformManager.generateWaveform(audioUrl);
    }
  }, [
    audioEditorManager.processedAudioUrl,
  ...
  const handleDragStart = useCallback(
    ...
    [isPlaying, currentTime, trim.startTime, trim.endTime, audioEditorManager]
  );
  ...
  const handleZoomIn = useCallback(() => {
    if (timelineZoom >= MAX_ZOOM) return;
    ...
    if (timelineContainerRef.current) {
      ...
      // Update zoom
      const newZoom = Math.min(MAX_ZOOM, timelineZoom + 0.5);
      ...
      setTimelineZoom(Math.min(MAX_ZOOM, timelineZoom + 0.5));
    }
  }, [
    timelineZoom,
    MAX_ZOOM,
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Subtitles visualization - if available */}
          {subtitles.length > 0 && (
            <div
          ...
          {activeEditMode === "trim" && (
            <>
              {/* Visual indicator for trimmed region */}
              <div
                className={`absolute top-8 h-16 ${
                  isDarkMode ? "bg-blue-500/20" : "bg-blue-500/10"
                }`}
                style={{
                  left: timeToPosition(trim.startTime),
                  width:
                    timeToPosition(trim.endTime) -
                    timeToPosition(trim.startTime),
                }}
              />
