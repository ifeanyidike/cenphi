...
interface RejectedCardViewProps {
  filteredTestimonials: Testimonial[];
  viewMode: 'card' | 'list';
  renderFormatIcon: (format: string) => React.ReactNode;
  renderStars: (rating: number) => React.ReactNode;
  handleActionClick: (testimonial: Testimonial, action: 'approve' | 'reject' | 'feature' | 'archive' | 'delete' | 'return_to_pending' | 'view_more') => void;
}

export const RejectedCardView: React.FC<RejectedCardViewProps> = ({
  ...
  const renderCardTestimonial = (testimonial: Testimonial) => {
    ...
    return (
      <div 
        ...
        className={cn(
          commonClasses, 
          "overflow-visible group shadow-sm hover:shadow-xl", // Changed overflow-hidden to overflow-visible
      ...
      >
        ...
        {testimonial.status === "rejected" && (
          <div className="absolute -right-11 top-5 text-blue-500 px-12 py-1 rotate-45 text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            Rejected
          </div>
        )}
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center 
            ...
            <div className="flex flex-wrap gap-2">
              <button 
                onClick={() => handleActionClick(testimonial, 'return_to_pending')}
                className="px-3 py-1.5 bg-amber-100 border border-amber-300 text-amber-700 rounded-lg 
                  hover:bg-amber-200 transition-colors flex items-center shadow-sm text-xs"
              >
                <RotateCcw className="w-3 h-3 mr-1" />
                Return to Pending
              </button>
              <button 
                onClick={() => handleActionClick(testimonial, 'delete')}
                className="px-3 py-1.5 bg-red-100 border border-red-300 text-red-700 rounded-lg 
                  hover:bg-red-200 transition-colors flex items-center shadow-sm text-xs"
              >
            ...
            <div className="relative">
              <button 
                onClick={(e) => toggleDropdown(e, testimonial.id)}
              ...
              {/* Dropdown menu */}
              {openDropdownId === testimonial.id && (
                <div 
                  className="absolute right-0 top-24 -translate-y-full w-48 bg-white rounded-lg shadow-lg py-1 z-50 border border-slate-200"
                  onClick={(e) => e.stopPropagation()}
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      handleActionClick(testimonial, 'view_more');
