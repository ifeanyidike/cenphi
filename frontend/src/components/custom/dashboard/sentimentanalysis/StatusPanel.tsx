...
// // Get sentiment info from analyses array
// const getSentimentFromAnalyses = (testimonial: Testimonial) => {
//   if (!testimonial.analyses || !Array.isArray(testimonial.analyses)) return null;
  
//   const sentimentAnalysis = testimonial.analyses.find(a => a.analysis_type === "sentiment");
//   if (!sentimentAnalysis) return null;
  
//   return {
//     score: (sentimentAnalysis.analysis_data as {sentiment?: {score?: number}})?.sentiment?.score || 
//            0,
//     label: (sentimentAnalysis.analysis_data as { sentiment?: { label?: string } })?.sentiment?.label || "neutral",
//     keywords: (sentimentAnalysis.analysis_data as { sentiment?: { keywords?: string[] } })?.sentiment?.keywords || []
//   };
// };
...
//   // Function to get the sentiment color
//   const getSentimentColor = (sentimentLabel: string | undefined): string => {
//     if (!sentimentLabel) return "bg-gray-300";
...
//   // Function to get sentiment icon based on label
...
//                   {selectedTestimonial.title || "Untitled"}
//                 </h3>
//                 {/* Updated to use the new sentiment structure */}
//                 {(() => {
//                   const sentiment = getSentimentFromAnalyses(selectedTestimonial);
//                   return sentiment ? (
//                     <div className="flex items-center mt-1">
//                       <div className={`w-6 h-6 rounded-full ${getSentimentColor(sentiment.label)} flex items-center justify-center mr-2`}>
...
//             {/* Display sentiment keywords if available */}
//             {(() => {
//               const sentiment = getSentimentFromAnalyses(selectedTestimonial);
//               return sentiment && sentiment.keywords && sentiment.keywords.length > 0 ? (
//                 <div className="mt-4 bg-slate-100 border-slate-200 p-3 rounded-lg border">
//                   <p className="text-xs text-slate-500 mb-1">SENTIMENT KEYWORDS</p>
...
// Get sentiment info from analyses array with improved typing
const getSentimentFromAnalyses = (testimonial: Testimonial): SentimentData | null => {
  if (!testimonial?.analyses || !Array.isArray(testimonial.analyses)) return null;
  
  const sentimentAnalysis = testimonial.analyses.find(a => a.analysis_type === "sentiment");
  if (!sentimentAnalysis || !sentimentAnalysis.analysis_data) return null;
  
  const analysisData = sentimentAnalysis.analysis_data;
  
  // More direct and type-safe access with nullish coalescing
  return {
    score: (analysisData.sentiment as { score?: number })?.score ?? sentimentAnalysis.sentiment_score ?? 0,
    label: (analysisData.sentiment as { label?: string })?.label ?? "neutral",
    keywords: (analysisData.sentiment as { keywords?: string[] })?.keywords ?? []
  };
};

// Function to format sentiment score as percentage
const formatSentimentScore = (score: number | undefined): string => {
...
export function StatusPanel({ 
  ...
  // Extract sentiment data for the selected testimonial
  const sentimentData = selectedTestimonial ? getSentimentFromAnalyses(selectedTestimonial) : null;
          ...
          <div className="bg-white border-slate-200 hover:border-indigo-300/70 rounded-lg p-5 border transition-all shadow-lg">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                {sentimentData && (
                  <div className="flex items-center mt-1">
                    <div className={`w-6 h-6 rounded-full ${getSentimentColor(sentimentData.label)} flex items-center justify-center mr-2`}>
            ...
            {/* Display narrative data if available */}
            {(() => {
              if (!selectedTestimonial?.analyses) return null;
              
              const narrativeAnalysis = selectedTestimonial.analyses.find(
                a => a.analysis_type === "narrative"
              );
              
              if (!narrativeAnalysis || !narrativeAnalysis.analysis_data) return null;
              ...
              return Array.isArray(themes) && themes.length > 0 ? (
                <div className="mt-4 bg-slate-100 border-slate-200 p-3 rounded-lg border">
                  ...
                  {narrativeData.narrative_strength && (
                    <p className="text-xs text-slate-500 mt-2">
                      Narrative Strength: <span className="font-medium capitalize">{String(narrativeData.narrative_strength)}</span>
                    </p>
                  )}
                </div>
              ) : null;
