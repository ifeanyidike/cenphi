...
import { ApprovedListView } from "@/components/custom/dashboard/approved/ApprovedListView";
import { ApprovedCardView } from "@/components/custom/dashboard/approved/ApprovedCardView";
...
export const ApprovedTestimonials: React.FC<ApproveReviewsProps> = ({ 
  testimonials, 
  onAction 
}) => {
  ...
  const [approvedfilterMenuOpen, setApprovedFilterMenuOpen] = useState(false);
  ...
  const filteredTestimonials = useMemo(() => {
    return testimonials.filter(testimonial => {
      ...
      if (activeFilters.length === 0) return testimonial.status === 'approved';
      ...
      return activeFilters.every((filter: string) => {
        switch (filter) {
          ...
          case "Today": {
            ...
            return testimonial.status === "approved" && today.toDateString() === createdDate.toDateString();
          }
          case "Week": {
            const today = new Date();
            const createdDate = new Date(testimonial.created_at);
            const oneWeekAgo = new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000);
            return testimonial.status === "approved" && createdDate >= oneWeekAgo && createdDate <= today;
          }
          case "Month": {
            const today = new Date();
            const createdDate = new Date(testimonial.created_at);
          ...
          case "Year": {
            const today = new Date();
            const createdDate = new Date(testimonial.created_at);
            return testimonial.status === "approved" && createdDate.getFullYear() === today.getFullYear();
          }
          ...
          case "Video":
          case "Audio":
            return testimonial.status === "approved" && testimonial.format.toLowerCase() === filter.toLowerCase();
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
          <div>
            ...
            <p className="text-gray-500 mt-2 text-sm sm:text-base">
              Review and manage your approved testimonials
            </p>
          </div>
          ...
          <div className="flex space-x-2">
            ...
            <StatusFilterMenu
              statusfilterMenuOpen={approvedfilterMenuOpen}
          ...
          <div className="bg-white rounded-2xl border border-gray-200/50 p-8 sm:p-12 text-center">
            <div className="flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-gray-500 max-w-md mx-auto mb-6">
                We couldn't find any approved testimonials matching your current filters. Try adjusting your criteria to see more results.
              </p>
