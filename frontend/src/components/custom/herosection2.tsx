        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-center">
          ...
          <div className="space-y-4 md:space-y-6 text-center md:text-left">
            ...
            <p className="text-base md:text-lg text-[#848FA5]">
              The all-in-one platform to collect, manage, and showcase
              testimonials that drive conversions and build trust with your
              audience.
            </p>
          ...
          <div className="relative h-[400px] md:h-[480px] bg-[#2D2D2A] rounded-2xl overflow-hidden mt-6 md:mt-0">
                ...
                <div
                  ...
                  style={{
                    ...
                    transform: `translate(-0%, -0%) ${
                      activeWidget === widget.id ? "scale(1.1)" : "scale(1)"
                    }`,
                  }}
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium text-xs md:text-sm text-[#2D2D2A]">
                        {widget.title}
                      </span>
                    </div>
                  </div>
                  <p className="text-xs md:text-sm text-[#848FA5] mb-2 line-clamp-2">
                    {widget.content}
                  </p>
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center space-x-1">
                      {[...Array(widget.rating)].map((_, i) => (
                        <Star
                          key={i}
                          className="w-2 md:w-3 h-2 md:h-3 text-[#C14953]"
                          fill="currentColor"
                        />
                      ))}
                    </div>
            ...
            <div className="absolute bottom-6 md:bottom-40 left-1/2 transform -translate-x-1/2 flex space-x-2 md:space-x-3 z-20">
              {floatingReviews.map((review) => (
                ...
                >
                  {review.icon}
                  <span className="text-[10px] md:text-xs font-semibold mt-1 text-[#2D2D2A]">
                    {review.label}
                  </span>
                </div>
              ))}
